...
function c9126351.excost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return c:GetFlagEffect(9126352)==0 and Duel.IsExistingMatchingCard(Card.IsAbleToHandAsCost,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RTOHAND)
	local g=Duel.SelectMatchingCard(tp,Card.IsAbleToHandAsCost,tp,LOCATION_MZONE,0,1,1,nil)
	Duel.SendtoHand(g,nil,REASON_COST)
	c:RegisterFlagEffect(9126352,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1)
end
