...
function c47355498.disfilter(c,im0,im1,re)
...
function c47355498.discheck(ev,category,re,im0,im1,targets)
	...
	if tg and #tg>0 then
		if targets and targets:IsContains(re:GetHandler()) then
			return tg:IsExists(c47355498.disfilter,1,nil,im0,im1,re)
		else
			return tg:IsExists(c47355498.disfilter,1,re:GetHandler(),im0,im1,re)
		end
	end
	return false
end
function c47355498.disop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not res and c47355498.discheck(ev,CATEGORY_TOEXTRA,re,im0,im1,targets) then res=true end
